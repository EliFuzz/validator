...
inline fun <reified T> T.validate(fieldName: String, fieldValue: Map<*, *>?): Validator<T> = object: Validator<T> {
    override fun exists(): Validator<T> =
        require(!fieldValue.isNullOrEmpty()) { "${prepend()} $fieldName must not be empty" }.let { this }
